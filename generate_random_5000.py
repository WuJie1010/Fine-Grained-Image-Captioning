"""
Generating captions for 5000 images
"""
...
if len(opt.id) == 0:
    opt.id = infos['opt'].id
ignore = ["id", "batch_size", "beam_size", "start_from", "language_eval", "input_json"]
...
# Set sample options
Sentencespath = os.path.join(path,'Sentence_random_5000.json')

if not os.path.exists(os.path.dirname(Sentencespath)):
    os.makedirs(os.path.dirname(Sentencespath))

_, split_predictions, _ = evalpro_utils.eval_split(model, crit, loader, vars(opt))

json.dump(split_predictions, open(Sentencespath, 'w'))
print("Save Sentences_Result finish!!!" )
