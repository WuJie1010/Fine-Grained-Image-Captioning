...
class CiderScorer(object):
    ...
    def compute_cider(self):
        def counts2vec(cnts):
            ...
            for (ngram,term_freq) in cnts.iteritems():
                ...
                vec[n][ngram] = float(term_freq)*(np.log(self.ref_len) - df)
        ...
        def sim(test_token, vec_hyp, vec_ref, norm_hyp, norm_ref, length_hyp, length_ref):
            '''
            ...
            #Get the token-level reward ,only the 1-ngram?

            for i, token in enumerate(test_token):
                if vec_hyp[0][token] > threshold:
                    token_score[i] = min(vec_hyp[0][token], vec_ref[0][token]) * vec_ref[0][token]
                if (norm_hyp[0] != 0) and (norm_ref[0] != 0):
                    token_score[i] /= (norm_hyp[0]*norm_ref[0])

                assert (not math.isnan(token_score[i]))
                token_score[i] *= np.e ** (-(delta ** 2) / (2 * self.sigma ** 2))
            ...
            for i, token in enumerate(test_token):
                ...
                for n in range(self.n):
                    ...
                    for (ngram, count) in vec_hyp[n].iteritems():
                        if token[0] in ngram:
                            if vec_hyp[n][ngram]>1:
                                token_val[n] += min(vec_hyp[n][ngram], vec_ref[n][ngram]) * vec_ref[n][ngram]

                    if (norm_hyp[n] != 0) and (norm_ref[n] != 0):
                        token_val[n] /= (norm_hyp[n]*norm_ref[n])
        ...
        for test, test_token, refs in zip(self.ctest, self.test_token, self.crefs):
            ...
            for ref in refs:
                ...
                val, token= sim(test_token, vec, vec_ref, norm, norm_ref, length, length_ref)
                score += val
                token_score += token
            #print(token_score)
    ...
    def compute_score(self, option=None, verbose=0):
        ...
        # compute cider score
        score, token_score = self.compute_cider()
        # debug
        # print score
        return np.mean(np.array(score)), np.array(score), np.array(token_score)
