"""
Holistic Discriminative Reward
"""
...
def get_hd_reward(vse, model, fc_feats, knn_fc_feats, data, gen_result, gen_result_baseline, loader):
    ...
    for i in range(batch_size):
        data.append((fc_feats[i], knn_fc_feats[i], gen_result[i], lengths_gen[i], i))
        # add i to readjust the order
    data.sort(key=lambda x: x[3], reverse=True) # sort according to sentence length
    img_feats, knn_img_feats, sample_result, lengths_gen, index = zip(*data) # get the reshape data

    img_feats = torch.stack(img_feats, 0)  # stack the image feature
    knn_img_feats = torch.stack(knn_img_feats, 0)  # stack the image feature
    ...
    knn_img_emb, cap_emb = vse.forward_emb(knn_img_feats, sample_result, lengths_gen) #embbing
    loss = vse.forward_knn_loss(img_emb, knn_img_emb, cap_emb) # get the loss as the reward
    ...
    greedy_res = torch.cat([gen_result_baseline.cpu(), zero],1)
    lengths_gre = np.zeros(batch_size, dtype=np.int)
    for i, cap in enumerate(greedy_res.numpy()):
        for j in range(len(cap)):
            if cap[j] == 0:
                lengths_gre[i] = j+1
                break
    ...
    gre_data = []
    for i in range(batch_size):
        gre_data.append((fc_feats[i], knn_fc_feats[i], greedy_res[i], lengths_gre[i], i))
    gre_data.sort(key=lambda x: x[3], reverse=True)
    img_feats, knn_img_feats, greedy_result, lengths_gre, index = zip(*gre_data)

    img_feats = torch.stack(img_feats, 0)
    knn_img_feats = torch.stack(knn_img_feats, 0)  # stack the image feature
    greedy_result = torch.stack(greedy_result, 0)

    # compute the embeddings
    img_emb, cap_emb = vse.forward_emb(img_feats, greedy_result, lengths_gre) #embbing
    knn_img_emb, cap_emb = vse.forward_emb(knn_img_feats, greedy_result, lengths_gre)
    gre_loss = vse.forward_knn_loss(img_emb, knn_img_emb, cap_emb) # get the loss as the reward

    # resort the data according to length number
    gre_redata = []
    for i in range(batch_size):
        gre_redata.append((img_feats[i], gre_loss[i], index[i]))
    ...
    print('Holistic Distinct scores:', np.mean(distinct_loss))
